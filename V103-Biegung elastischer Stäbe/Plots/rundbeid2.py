...
x, D0, Dm = np.genfromtxt('rundbeid2.txt', unpack=True)
x *= 1e-2
D0 *= 1e-3
Dm *= 1e-3
...
# np.savetxt('test4.txt', np.column_stack([x, D0, Dm, D(x), F(x)]), fmt="%2.2f", header="x D0 Dm D F")
...
plt.xlabel(r'$\left(4 x^3 - 12 L x^2 + 9  L^2  x - L^3\right) \,/\, \mathrm{m^3}$')
